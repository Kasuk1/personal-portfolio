import {
  CSSLogo,
  FigmaLogo,
  HtmlLogo,
  JavascriptLogo,
  NodeLogo,
  ReactLogo,
} from 'components/svg';
import { IExperienceType, SkillTechType } from 'types/skill';

export const skillTechData: SkillTechType[] = [
  {
    skillLevel: 'advanced',
    skillName: 'Javascript/Typescript',
    logo: JavascriptLogo,
  },
  { skillLevel: 'advanced', skillName: 'HTML', logo: HtmlLogo },
  { skillLevel: 'advanced', skillName: 'CSS', logo: CSSLogo },
  { skillLevel: 'advanced', skillName: 'React/Redux', logo: ReactLogo },
  { skillLevel: 'intermediate', skillName: 'Node/Express', logo: NodeLogo },
  { skillLevel: 'intermediate', skillName: 'Figma', logo: FigmaLogo },
];

export const experienceData: IExperienceType[] = [
  {
    date: 'Mar 2022 - Mar 2023',
    companyName: 'Sharpminds',
    role: 'React Developer',
    pageUrl: 'https://sharpminds.com/en/',
    description: [
      'Implement new interfaces based on UI-Design predefined.',
      'Migrate code functionality to React environment following current practices and applying new ones.',
      'Refactor and deprecate old code to React environment.',
    ],
  },
  {
    date: 'Dec 2021 - Mar 2022',
    companyName: 'Pragma',
    role: 'Software Engineer',
    pageUrl: 'https://www.pragma.co/es/',
    description: [
      'Developed web applications in the front-end environment with different technologies and methodologies: React, Typescript, Redux Toolkit, Sass, React-Router-v6, React-Hook-Form, TDD.',
      'Worked with agile methodologies like Scrum to deliver a high quality product developed and tested.',
      'Guided new peers in projects and did inspections to show the current project’s state.',
    ],
  },
  {
    date: 'Sep 2021 - Nov 2021',
    companyName: 'NTT Data',
    role: 'Centers Pool',
    pageUrl: 'https://www.nttdata.com/global/en/',
    description: [
      'Developed web applications in the front-end environment with Angular, RxJS, Sass.',
      'Worked with peers with different background  to enhance productivity and efficiency with agile methodologies.',
    ],
  },
];
